...
public class dbload {
	...
	public void readFile(String filename, int pagesize) throws IOException
	{
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				...
				String toffset=checkPageNumber(numberOfPagesUsed)+"_"+checkRecNumber(pageCount);
				
				
				_bt.insert((sdtName),  toffset);
				pageCount+=1;
			}
		...
		// save b+ tree to disk
		if (constants.SAVE_TREE_TO_DISK)
			doSaveTreeToDisk();
	...
	{
		String ll = Integer.toString(recNum);
		if(ll.length()==1)
			ll="0"+ll;
	...
	public static void doSaveTreeToDisk() {

		FileOutputStream fosTree = null;

		try {
			fosTree = new FileOutputStream(constants.BPLUS_TREE_FILE_NAME);
